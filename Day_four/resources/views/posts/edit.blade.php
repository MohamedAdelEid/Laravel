...
@section('content')
    <header class="mb-10">
        <h1 class="text-3xl font-bold text-gray-900 sm:text-4xl">Edit Post</h1>
        <p class="mt-1.5 text-sm text-gray-500">Update the details of your post.</p>
    </header>
        ...
        <form action="{{ route('posts.update', $post->id) }}" method="POST" class="space-y-6" enctype="multipart/form-data">
                ...
                <select name="user_id" id="user_id" required
                    ...
                    @foreach ($users as $user)
                        <option value="{{ $user->id }}"
                            {{ old('user_id', $post->user_id) == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                    @endforeach
                </select>
                @error('user_id')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
            ...
            <div>
                ...
                <textarea name="body" id="body" rows="6" required
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm px-4 py-2 border">{{ old('body', $post->body) }}</textarea>
                @error('body')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <x-image-upload name="image" :currentImage="$post->image_url" />
            </div>
